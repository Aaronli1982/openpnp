...
public class MountsmdUlpImporter extends JPanel implements BoardImporter {
    private Frame owner;
    ...
    public Board importBoard() throws Exception {
        dialog = new JDialog(owner, "Import EAGLE mountsmd.ulp Files", true);

        // This stuff wires up the escape key to the cancel action.
        ...
        // When this call returns the user has either finished the import or
    ...
    private static List<Placement> parseFile(File file, Side side,
        ...
        while ((line = reader.readLine()) != null) {
            ...
            Pattern pattern = Pattern
                    .compile("(\\S+)\\s+(\\d+\\.\\d+)\\s+(\\d+\\.\\d+)\\s+(\\d{1,3})\\s(.*?)\\s(.*)");
            Matcher matcher = pattern.matcher(line);
            matcher.matches();
            Placement placement = new Placement(matcher.group(1));
            placement.setLocation(new Location(LengthUnit.Millimeters, Double
                    .parseDouble(matcher.group(2)), Double.parseDouble(matcher
                    .group(3)), 0, Double.parseDouble(matcher.group(4))));
    ...
    private class SwingAction_2 extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            File topFile = new File(textFieldTopFile.getText());
            File bottomFile = new File(textFieldBottomFile.getText());
            board = new Board();
            ...
            try {
                ...
                MessageBoxes.errorBox(MountsmdUlpImporter.this, "Import Error",
                        e1);
                return;
            }
